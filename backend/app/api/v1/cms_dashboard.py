...
def dashboard_stats(
    ...
    published_today = [Article.status == ArticleStatus.PUBLISHED, Article.published_at >= today]
    ...
    def top(column, limit: int = 5) -> list[dict]:
        ...
        rows = db.execute(
            ...
            .where(*base, Article.status == ArticleStatus.PUBLISHED,
                   Article.published_at >= week, column.is_not(None))
            .group_by(column).order_by(func.count(Article.id).desc()).limit(limit)
        ).all()
    ...
    def label(model, rows: list[dict]) -> list[dict]:
        if not rows:
            return []
        names = {r.id: r for r in db.scalars(
            select(model).where(model.id.in_([r["id"] for r in rows]))).all()}
        return [{**r, "name_te": names[r["id"]].name_te, "name_en": names[r["id"]].name_en}
                for r in rows if r["id"] in names]

    views_today = int(db.scalar(
        select(func.count(ArticleEvent.id)).where(
            ArticleEvent.event_type == EventType.VIEW,
            ArticleEvent.created_at >= today)) or 0)
    active_users = int(db.scalar(
        select(func.count(func.distinct(ArticleEvent.user_id))).where(
            ArticleEvent.created_at >= week, ArticleEvent.user_id.is_not(None))) or 0)
    ...
    return DashboardStats(
        ...
        total_users=int(db.scalar(select(func.count(User.id)).where(User.deleted_at.is_(None))) or 0),
        active_users_7d=active_users,
        views_today=views_today,
        submissions_pending=int(db.scalar(select(func.count(CreatorSubmission.id)).where(
            CreatorSubmission.status == SubmissionStatus.PENDING)) or 0),
        ai_suggestions_new=int(db.scalar(select(func.count(AiSuggestion.id)).where(
            AiSuggestion.status == AiSuggestionStatus.NEW)) or 0),
        ai_drafts_pending=int(db.scalar(select(func.count(AiArticleDraft.id)).where(
            AiArticleDraft.status == AiDraftStatus.DRAFT)) or 0),
...
def editor_options(db: Session = Depends(get_db),
                   _principal: Principal = Depends(_EDITOR)) -> CmsEditorOptions:
    ...
    categories = db.scalars(select(Category).where(Category.is_active.is_(True))
                            .order_by(Category.sort, Category.name_en)).all()
    districts = db.scalars(select(District).where(District.is_active.is_(True))
                           .order_by(District.state, District.sort, District.name_en)).all()
    states = db.scalars(select(State).where(State.is_active.is_(True))
                        .order_by(State.sort)).all()
    authors = db.scalars(select(User).where(User.deleted_at.is_(None), User.is_author.is_(True))
                         .order_by(User.name_en).limit(200)).all()
    tags = db.scalars(select(Tag).where(Tag.is_active.is_(True))
                      .order_by(Tag.usage_count.desc(), Tag.name_en).limit(200)).all()
    return CmsEditorOptions(
        categories=[CmsCategoryOption(id=x.id, slug=x.slug, name_te=x.name_te,
                                      name_en=x.name_en, parent_id=x.parent_id)
                    for x in categories],
        districts=[CmsDistrictOption(id=x.id, slug=x.slug, name_te=x.name_te,
                                     name_en=x.name_en, state=x.state) for x in districts],
        states=[CmsOption(id=x.id, slug=x.slug, name_te=x.name_te, name_en=x.name_en)
                for x in states],
        authors=[{"id": x.id, "name_te": x.name_te, "name_en": x.name_en} for x in authors],
        tags=[CmsTagRef(id=x.id, slug=x.slug, name_te=x.name_te, name_en=x.name_en) for x in tags],
    )


@router.get("/mandals")
def editor_mandals(district_id: int = Query(..., ge=1), db: Session = Depends(get_db),
...
def editor_localities(mandal_id: int = Query(..., ge=1), db: Session = Depends(get_db),
                      _principal: Principal = Depends(_EDITOR)):
    ...
    return {"items": [{"id": x.id, "slug": x.slug, "name_te": x.name_te,
                       "name_en": x.name_en, "kind": x.kind} for x in rows]}
