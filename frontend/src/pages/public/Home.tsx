...
function SectionRule({ title, to }: { title: string; to?: string }) {
  ...
  return (
    <div className="mb-3 flex items-baseline justify-between border-b-2 border-ink pb-1.5">
      <h2 className={`${script} text-[19px] font-extrabold text-brand`}>
        {title}
      </h2>
      {to ? (
        <Link
...
function ForYouBlock() {
  ...
  return (
    <section className="mt-8">
      ...
      <p
        className={`${language === "te" ? "te" : "font-sans"} mt-2 text-[11px] text-muted-light`}
      >
        {language === "te"
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 py-4" aria-busy="true">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr_0.8fr]">
        <div>
          <div
            className="ph animate-pulse rounded-[4px]"
        ...
        <div className="flex flex-col gap-3">
          {[0, 1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-10 animate-pulse rounded bg-placeholder"
            />
          ))}
        </div>
      </div>
...
function ErrorState({
  ...
  return (
    <div className="mx-auto max-w-[680px] px-4 py-16 text-center">
      <h2
        className={`${language === "te" ? "th" : "font-sans"} text-[20px] font-bold text-ink`}
      >
        {t("state.newsFailed")}
      </h2>
...
export default function Home() {
  ...
  return (
    <main className="mx-auto max-w-[1200px] px-4 pb-8 pt-4">
      ...
      >
        <span
          className={`${script} sticky left-0 shrink-0 bg-white pr-1 text-[11px] font-extrabold uppercase tracking-[0.08em] text-breaking`}
        >
      ...
      {data.epaper ? (
        <section className="mb-7 overflow-hidden rounded-card border-2 border-brand bg-[#fffaf0] shadow-card">
          <div className="grid items-stretch md:grid-cols-[.72fr_1.28fr]">
            <div className="flex min-h-52 items-center justify-center bg-ink p-6 text-center text-white">
              <div>
                <NewspaperMark />
                <p className="th mt-3 text-3xl font-extrabold">
                  ఈ రోజు తెలుగు వార్తలు
                </p>
                <p className="mt-2 font-sans text-xs uppercase tracking-[.2em]">
                  {data.epaper.pub_date} · {data.epaper.page_count} pages
                </p>
              </div>
            </div>
            <div className="p-6">
              <p className="font-sans text-xs font-extrabold uppercase tracking-[.15em] text-brand">
                Today’s Top Telugu News — E-Paper
              </p>
              <h2 className="th mt-2 text-3xl font-extrabold">
                వార్తాపత్రికలా చదవండి. రేడియోలా వినండి.
              </h2>
              <div className="mt-5 grid gap-2 sm:grid-cols-2">
                <Link
                  className="rounded-control bg-brand px-4 py-3 text-center font-bold text-white"
                  to={`/epaper/${data.epaper.pub_date}`}
                >
                  Read like a newspaper
                </Link>
                <Link
                  className="rounded-control border border-brand px-4 py-3 text-center font-bold text-brand"
                  to={`/epaper/${data.epaper.pub_date}#radio`}
                >
                  Listen like radio
                </Link>
                <button
                  className="rounded-control border px-4 py-3 font-bold"
                  onClick={() =>
                    navigator.share?.({
                      title: "Today's Telugu News",
                      url: `${location.origin}/epaper/${data.epaper!.pub_date}/page/1`,
                    })
                  }
                >
                  Share page
                ...
                <a
                  className="rounded-control border px-4 py-3 text-center font-bold"
                  href={`/api/v1/epaper/${data.epaper.pub_date}/pdf`}
                >
                  Download today’s edition
                </a>
                <Link
                  className="rounded-control bg-ink px-4 py-3 text-center font-bold text-white sm:col-span-2"
                  to="/my-epaper"
                >
                  Create my E-Paper
                </Link>
      ...
      {(topics.data?.items.length ?? 0) > 0 ? (
        <section className="mb-7">
          <SectionRule
            title={
              language === "te"
                ? "🔥 ప్రజలు మాట్లాడుకుంటున్న టాప్ 3 అంశాలు"
                : "🔥 Top 3 issues people are talking about"
            }
          />
          <div className="grid gap-3 sm:grid-cols-3">
            {topics.data!.items.map((topic, i) => (
              <Link
                key={topic.slug}
                to={`/topic/${topic.slug}`}
                className="rounded-card border bg-white p-4 shadow-card hover:border-brand"
              >
                <span className="font-sans text-2xl font-black text-brand">
                  {i + 1}
                </span>
                <h3 className="th mt-1 text-xl font-extrabold">
                  {pick(topic.title_te, topic.title_en)}
                </h3>
                <p className="mt-2 text-xs text-muted">
                  {topic.is_override ? "Editor selected" : "Trending now"} →
                </p>
              </Link>
            ))}
          </div>
        ...
        <div className="min-w-0">
          ...
          {data.briefs.length > 0 ? (
            <div className="mt-4 border-t border-rule pt-3">
              <p className={`${script} mb-2 text-[11px] font-bold text-brand`}>
                {t("home.briefs")}
              </p>
        ...
        <aside className="min-w-0">
          ...
          <section className="mt-6 border border-rule bg-paper p-3 text-center">
            ...
            <div className="ph border border-rule" style={{ height: 160 }}>
              <span className={`${script} text-[11px]`}>
                {t("home.todaysPage")}
              </span>
            </div>
            <Link
...
function NewspaperMark() {
  return (
    <div
      aria-hidden
      className="mx-auto flex h-16 w-16 items-center justify-center rounded-full border-2 border-white text-4xl"
    >
      📰
    </div>
  );
