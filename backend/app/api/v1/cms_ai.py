...
def _suggestion_row(s: AiSuggestion) -> dict:
    return {
        "id": s.id, "topic_te": s.topic_te, "topic_en": s.topic_en,
        "rationale_te": s.rationale_te, "category_id": s.category_id,
        "district_id": s.district_id, "status": s.status, "score": round(s.score, 2),
        "engine": s.engine, "model": s.model, "created_at": s.created_at,
        "review_note": s.review_note,
        # §17: attribution travels with the suggestion so the editor can check
        # the claim before a word is written.
        "sources": [{"publisher": x.publisher, "title": x.title, "url": x.url,
                     "licence": x.licence, "excerpt": x.excerpt} for x in s.sources],
    }


def _draft_row(d: AiArticleDraft) -> dict:
    return {"id": d.id, "suggestion_id": d.suggestion_id, "title_te": d.title_te,
            "summary_te": d.summary_te, "body": d.body, "body_plain": d.body_plain,
            "category_id": d.category_id, "district_id": d.district_id,
            "status": d.status, "engine": d.engine, "model": d.model,
            "confidence": d.confidence, "word_count": d.word_count,
            "article_id": d.article_id, "created_at": d.created_at}


@router.get("/suggestions")
def list_suggestions(status: AiSuggestionStatus | None = None,
    ...
    rows = db.scalars(stmt.order_by(AiSuggestion.score.desc(), AiSuggestion.created_at.desc())
                      .offset(offset).limit(limit)).all()
    return {"items": [_suggestion_row(s) for s in rows], "total": total}


@router.post("/suggestions/generate", status_code=201)
def generate(payload: GenerateIn, request: Request, db: Session = Depends(get_db),
    ...
    audit_service.record(db, action=AuditAction.AI_RUN, entity_type="ai_suggestion",
                         entity_id="generate", actor=p.user,
                         after={"count": len(created)}, request=request)
    return {"items": [_suggestion_row(s) for s in created], "total": len(created)}


@router.post("/suggestions/{suggestion_id}/reject")
def reject(suggestion_id: int, payload: RejectIn, request: Request,
           db: Session = Depends(get_db),
           p: Principal = Depends(require_permission("ai.use"))):
    suggestion = ai_service.reject_suggestion(db, suggestion_id, actor_id=p.id, note=payload.note)
    audit_service.record(db, action=AuditAction.REJECT, entity_type="ai_suggestion",
                         entity_id=suggestion.id, actor=p.user, note=payload.note, request=request)
    return _suggestion_row(suggestion)


@router.post("/suggestions/{suggestion_id}/draft", status_code=201)
def draft(suggestion_id: int, payload: DraftIn, request: Request,
          db: Session = Depends(get_db),
          p: Principal = Depends(require_permission("ai.use"))):
    row = ai_service.create_draft(db, suggestion_id, actor_id=p.id, notes=payload.notes)
    audit_service.record(db, action=AuditAction.AI_RUN, entity_type="ai_article_draft",
                         entity_id=row.id, actor=p.user, request=request)
    return _draft_row(row)


@router.get("/drafts")
def list_drafts(status: AiDraftStatus | None = None,
                offset: int = Query(0, ge=0), limit: int = Query(50, ge=1, le=100),
                db: Session = Depends(get_db),
                _p: Principal = Depends(require_permission("ai.use"))):
    ...
    total = int(db.scalar(select(func.count()).select_from(stmt.subquery())) or 0)
    rows = db.scalars(stmt.order_by(AiArticleDraft.created_at.desc())
                      .offset(offset).limit(limit)).all()
...
def convert(draft_id: int, request: Request, db: Session = Depends(get_db),
...
def discard(draft_id: int, request: Request, db: Session = Depends(get_db),
            p: Principal = Depends(require_permission("ai.use"))):
    row = ai_service.discard_draft(db, draft_id, actor_id=p.id)
    audit_service.record(db, action=AuditAction.DELETE, entity_type="ai_article_draft",
                         entity_id=row.id, actor=p.user, request=request)
    return _draft_row(row)
