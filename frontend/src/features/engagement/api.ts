...
export async function fetchComments(shortId: string): Promise<CommentList> {
  const { data } = await api.get<CommentList>(`/public/articles/${shortId}/comments`);
  return data;
}
...
export async function reportArticle(
  ...
  await api.post(`/articles/${shortId}/report`, { reason, note: note || null });
}
...
export async function fetchBookmarks(offset = 0): Promise<{
...
export async function fetchHistory(offset = 0): Promise<{
  items: HistoryItem[];
  next_offset: number | null;
}> {
...
export async function fetchFollowingFeed(offset = 0): Promise<{
  articles: ArticleCard[];
  next_offset: number | null;
}> {
...
export async function fetchForYou(offset = 0, limit = 20): Promise<{
  articles: ArticleCard[];
  next_offset: number | null;
}> {
