...
_SCALARS = (
    "category_id", "subcategory_id", "district_id", "mandal_id", "locality_id",
...
_TEXT = ("title_en", "sub_title_te", "summary_te", "byline_te", "source_credit",
...
def _resolve_placement(db: Session, article: Article, values: dict[str, Any]) -> None:
    ...
    if article.mandal_id is not None:
        ...
        if article.district_id is None:
            ...
            raise ValidationError(details={"mandal_id": "does not belong to the selected district"})
    if article.locality_id is not None:
        ...
        if article.mandal_id is None:
            ...
            raise ValidationError(details={"locality_id": "does not belong to the selected mandal"})
...
def _apply_tags(db: Session, article: Article, names: list[str]) -> None:
    ...
    for raw in names:
        ...
        if not name:
            continue
        slug = slugify(name)[:100] or slugify(name.encode("ascii", "ignore").decode())[:100]
...
def _apply_gallery(db: Session, article: Article, media_ids: list[int]) -> None:
    ...
    known = {m.id for m in db.scalars(
        select(Media).where(Media.id.in_(media_ids), Media.deleted_at.is_(None))).all()
    } if media_ids else set()
    missing = [i for i in media_ids if i not in known]
    if missing:
        raise ValidationError(details={"gallery_media_ids": f"unknown media: {missing}"})

    db.execute(delete(ArticleMedia).where(ArticleMedia.article_id == article.id,
                                          ArticleMedia.role == "gallery"))
    for position, media_id in enumerate(dict.fromkeys(media_ids)):
        db.add(ArticleMedia(article_id=article.id, media_id=media_id,
                            role="gallery", sort=position))
...
def apply_copy(article: Article, values: dict[str, Any], db: Session | None = None) -> None:
    ...
    for key in ("subcategory_id", "district_id", "mandal_id", "locality_id",
...
def _guard_flags(db: Session, principal: Principal, article: Article,
...
def transition(db: Session, principal: Principal, article: Article, action: str,
               note: str | None, scheduled_at: datetime | None = None) -> Article:
    _scope(principal, article)
    ...
    mapping = {
        ...
        "publish": ({WorkflowState.APPROVED, WorkflowState.SCHEDULED}, WorkflowState.PUBLISHED),
    ...
    if action == "publish":
        ...
        when = scheduled_at if old == WorkflowState.SCHEDULED else (
            scheduled_at or article.scheduled_at
        )
        if when is not None and when > utcnow():
            article.scheduled_at = when
            article.status = ArticleStatus.SCHEDULED
            article.workflow_state = WorkflowState.SCHEDULED
            db.add(WorkflowTransition(article_id=article.id, from_state=old,
                                      to_state=WorkflowState.SCHEDULED, actor_id=principal.id,
        ...
        try:
            ...
            get_logger(__name__).exception("tts_pregenerate_failed", article_id=article.id)
